...
for i in range(seed_count):
	point_list.append(( r.randint(kernel_size, width-kernel_size-1), r.randint(kernel_size, height-kernel_size-1), (50+ 10*i, 200, 200), 90 , (r.random() - r.random(), r.random() - r.random()), branching_factor, 0))
...
while point_list:
	...
	if branching_timer >= branching_speed and branches > 0:
		branching_timer = 0
		branches -= 1
		point_list.append( (x, y , (h- 2*seed_count, s, v), sen_counter+senoidal_update, direction, branches, 0) )
	...
	if  kernel_size < x < width - kernel_size and kernel_size < y < height - kernel_size : 
		...
		if  kernel_size < x < width - kernel_size and kernel_size < y < height - kernel_size : 
			...
			point_list.append( (int(x_n), int(y_n) , (h, s, v), sen_counter+senoidal_update, direction, branches, (branching_timer+1) ) )
